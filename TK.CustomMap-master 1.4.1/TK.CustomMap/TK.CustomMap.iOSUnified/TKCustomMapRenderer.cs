...
namespace TK.CustomMap.iOSUnified
{
    ...
    public class TKCustomMapRenderer : MapRenderer, IRendererFunctions
    {
        ...
        private TKCustomMap FormsMap => this.Element as TKCustomMap;
        ...
        private void OnPinPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (e.PropertyName == TKCustomMapPin.TitlePropertyName ||
                e.PropertyName==TKCustomMapPin.IdPropertyName||
                e.PropertyName == TKCustomMapPin.TypePropertyName ||
        ...
        private async void UpdateImage(MKAnnotationView annotationView, TKCustomMapPin pin)
        {
            try
            {
                if (pin.Image != null)
                {
                    ...
                    //var image = (await pin.Image.ToImage()).Scale(new SizeF { Height = 60, Width = 60 });
                    var image = (await pin.Image.ToImage());
                    var scaledImage = image.Scale(new CGSize(image.Size.Width/2, image.Size.Height/2), new nfloat(2));
                    Device.BeginInvokeOnMainThread(() =>
                    {
                        annotationView.CenterOffset = new CGPoint(0, -scaledImage.Size.Height/2);
        ...
        private void UpdateTileOptions()
        {
            ...
            if(this.FormsMap != null && this.FormsMap.TilesUrlOptions != null)
            {
                        ...
                        .Replace("{2}", "{z}"))
                {
                    ...
                    MinimumZ = this.FormsMap.TilesUrlOptions.MinimumZoomLevel,
                    MaximumZ = this.FormsMap.TilesUrlOptions.MaximumZoomLevel,
        ...
        private void SetSelectedPin()
        {
                ...
                if (this.FormsMap.SelectedPin != null)
                {
                    ...
                    if (selectedAnnotation != null)
                    {
                        ...
                        if (this.FormsMap.PinSelectedCommand != null &&
                            this.FormsMap.PinSelectedCommand.CanExecute(null))
                        {
    ...
    {
        private static UIImage FromUrl(this UIImage target, string uri)
        {
