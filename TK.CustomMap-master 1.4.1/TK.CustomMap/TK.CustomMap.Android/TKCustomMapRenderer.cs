...
using Font = Map_Bul_App.Droid.Font;
...
namespace TK.CustomMap.Droid
{
    ...
    public class TKCustomMapRenderer : MapRenderer, IRendererFunctions, IOnMapReadyCallback, GoogleMap.ISnapshotReadyCallback, GoogleMap.IInfoWindowAdapter
    {
        ...
        private void OnMarkerClick(object sender, GoogleMap.MarkerClickEventArgs e)
        {
            ...
            {
                 Console.WriteLine("TKDEBUG: FormsMap is null");
                return;
            }
            var item = this._markers.SingleOrDefault(i => i.Value.Id.Equals(e.Marker.Id));
            if (item.Key == null)
            {
                 Console.WriteLine("TKDEBUG: CustomPin is null");
                return;
            }
            ...
            this.FormsMap.SelectedPin = item.Key;
             Console.WriteLine("TKDEBUG: All is OK "+item.Key.Id);
        ...
        {
            if (this.FormsMap == null || FormsMap.MapLongPressCommand == null) return;
        ...
        private async void OnPinPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            {
                case TKCustomMapPin.IdPropertyName:
                    //marker.Id = pin.Id;
                    break;
                case TKCustomMapPin.TypePropertyName:
                    //marker.Id = pin.Id;
                    break;
                case TKCustomMapPin.InfoImagePropertyName:
                    //marker.Id = pin.Id;
                    break;
                case TKCustomMapPin.IsSmallPropertyName:
                    //marker.Id = pin.Id;
                    break;
                case TKCustomMapPin.ScalePropertyName:
                    //marker.Id = pin.Id;
                    break;
        ...
        {
             Console.WriteLine("TKDEBUG: Work in SetSelectedItem()");
            if (this._selectedMarker != null)
            {
                this._selectedMarker.HideInfoWindow();
                this._selectedMarker = null;
                 Console.WriteLine("TKDEBUG: HideInfoWindow()");
            }
            if (this.FormsMap.SelectedPin != null)
            {
                if (!this._markers.ContainsKey(this.FormsMap.SelectedPin))
                {
                     Console.WriteLine("TKDEBUG: _markers not contain SelectedPin");
                    return;
                }
                ...
                Console.WriteLine("SelectedPinShowCallout: "+FormsMap.SelectedPin.ShowCallout);
                if (this.FormsMap.SelectedPin.ShowCallout)
                {
                     Console.WriteLine("TKDEBUG: Showing InfoWindow");
        ...
        private async Task UpdateImage(TKCustomMapPin pin, MarkerOptions markerOptions)
        {
            ...
            try
            {
                if (pin.Image != null)
                {
                    var image = await pin.Image.ToBitmap(this.Context);
                    if (!pin.IsSmall)
                    {
                        bitmap = BitmapDescriptorFactory.FromBitmap(image);
                    }
                    else
                    {
                        var newimage = Bitmap.CreateScaledBitmap(image, image.Width/pin.Scale, image.Height/pin.Scale, true);
                        image.Recycle();
                        bitmap = BitmapDescriptorFactory.FromBitmap(newimage);
                    }
                }
                else
                {
        ...
        public Android.Views.View GetInfoContents(Marker marker)
          {
            /*var inflater = Android.App.Application.Context.GetSystemService(Context.LayoutInflaterService) as Android.Views.LayoutInflater;
              ...
              var customPin = GetPinByMarker(marker);
              if (customPin == null)
              {
                  throw new Exception("Custom pin not found");
              }
              view = inflater.Inflate(Resource.Layout.MapInfoWindowMy, null, true);
            
              var infoTitle = view.FindViewById<TextView>(Resource.Id.InfoWindowTitle);
              var typeLabel = view.FindViewById<TextView>(Resource.Id.InfoTextType);
            var type2Label = view.FindViewById<TextView>(Resource.Id.InfoTextType2);
            var timeLabel = view.FindViewById<TextView>(Resource.Id.InfoTextTime);
            var tagsLabel = view.FindViewById<TextView>(Resource.Id.InfoTextTags);
            // var webView = view.FindViewById<Android.Webkit.WebView>(Resource.Id.WebViewWindowSubtitle);
            //webView.LoadData();


            var image = view.FindViewById<ImageView>(Resource.Id.TitleImage);

            if (image != null)
            {
                var imageUrl = customPin.InfoImage;
                if (!string.IsNullOrEmpty(imageUrl))
                {
                    try
                    {
                        var imageFromUrl = GetImageBitmapFromUrl(imageUrl);
                        
                        image.SetImageBitmap(imageFromUrl);
              ...
              /*var  infoTextHtml = Html.FromHtml("<p>Ресторан</p>" +
                                      "<p><b>Кухня:</b> японская, европейская</p>" +
                                      "<p><b>Режим работы:</b> 12:00 - 23:00</p>" +
                                      "<p><b>Район:</b> Jimbaran</p>" +
                                      "<p>+62 (361) 123 456</p>" +
                                      "<p>jl. By Pass Ngurah Rai</p>" +
                                      "<p>delivery, free Wi-Fi, X-island discount</p>");
            const string infoText =   "Ресторан \n " +
                                      "Кухня: японская, европейская\n " +
                                      "Режим работы: 12:00 - 23:00\n " +
                                      "Район: Jimbaran\n" +
                                      "+62 (361) 123 456\n" +
                                      "jl. By Pass Ngurah Rai\n" +
                                      "delivery, free Wi-Fi, X-island discount";#1#

            if (infoTitle != null)
            {
                infoTitle.SetText(customPin.Title, TextView.BufferType.Normal);
                infoTitle.Typeface = Font.RobotoLight;
                infoTitle.SetBackgroundColor(customPin.DefaultPinColor.ToAndroid());
                infoTitle.SetPadding(20,0,0,0);
                infoTitle.TextSize = 20f;
            }
            if (typeLabel != null)
            {
                typeLabel.SetText(customPin.Subtitle, TextView.BufferType.Normal);
                typeLabel.Typeface = Font.RobotoLight;
                typeLabel.SetTextColor(customPin.DefaultPinColor.ToAndroid());
                typeLabel.SetPadding(20, 0, 20, 0);
            }

            if (type2Label != null)
            {
                type2Label.Typeface = Font.RobotoLight;
                type2Label.SetPadding(20, 0, 20, 0);
            }
            if (timeLabel != null)
            {
                timeLabel.Typeface = Font.RobotoBold;
                timeLabel.SetPadding(20, 0, 20, 0);
            }
            if (tagsLabel != null)
            {
                tagsLabel.SetTextColor(Android.Graphics.Color.Gray);
                tagsLabel.TextSize = 10f;
                tagsLabel.Typeface = Font.RobotoMedium;
                tagsLabel.SetPadding(20, 0, 20, 0);
                //tagsLabel.SetTextSize(ComplexUnitType.Sp, 14f);
            }
            view.SetPadding(0,0,0,0);
            //infoSubtitle.SetText(Html.FromHtml(infoTextHtml), TextView.BufferType.Normal);
            return view;*/
            return null;
          }
        ...
        public Android.Views.View GetInfoWindow(Marker marker)
        {
            ...
            var view = inflater.Inflate(Resource.Layout.MapInfoWindowBordered, null, true);
            var infoTitle = view.FindViewById<TextView>(Resource.Id.InfoWindowTitle);
            var typeLabel = view.FindViewById<TextView>(Resource.Id.InfoTextType);
            var type2Label = view.FindViewById<TextView>(Resource.Id.InfoTextType2);
            var timeLabel = view.FindViewById<TextView>(Resource.Id.InfoTextTime);
            var tagsLabel = view.FindViewById<TextView>(Resource.Id.InfoTextTags);
            var imageView = view.FindViewById<ImageView>(Resource.Id.TitleImage);
            ...
            if (infoTitle != null)
            {
                ...
                infoTitle.Typeface = Font.RobotoLight;
                infoTitle.SetBackgroundColor(customPin.DefaultPinColor.ToAndroid());
                infoTitle.SetPadding(20, 0, 20, 0);
                infoTitle.TextSize = 20f;
            }
            if (typeLabel != null)
            {
            ...
            if (type2Label != null)
            {
                type2Label.SetText(customPin.Type, TextView.BufferType.Normal);
                type2Label.Typeface = Font.RobotoLight;
                type2Label.SetPadding(20, 0, 20, 0);
            }
            if (timeLabel != null)
            {
                timeLabel.SetText(customPin.WorkTime, TextView.BufferType.Normal);
                timeLabel.Typeface = Font.RobotoBold;
                timeLabel.SetPadding(20, 0, 20, 0);
            }
            if (tagsLabel != null)
            {
                tagsLabel.SetText(customPin.Tags, TextView.BufferType.Normal);
                tagsLabel.SetTextColor(Android.Graphics.Color.Gray);
                tagsLabel.TextSize = 10f;
                tagsLabel.Typeface = Font.RobotoMedium;
                tagsLabel.SetPadding(20, 0, 20, 0);
        ...
        private void View_LayoutChange(object sender, LayoutChangeEventArgs e)
        {
            try
            {
